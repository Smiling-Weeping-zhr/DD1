...
def get_images(args, model_teacher, hook_for_display, ipc_id):
    ...
    for kk in range(0, 1000, batch_size):
        ...
        for iteration in range(iterations_per_layer):
            ...
            # ipdb.set_trace()
            loss_r_bn_feature = sum([mod.r_feature.to('cuda:2') * rescale[idx] for (idx, mod) in enumerate(loss_r_feature_layers)])
            ...
            # ipdb.set_trace()
            # combining losses
            t_device = loss_r_bn_feature.device

            loss_aux = args.tv_l2 * loss_var_l2 + \
                        args.l2_scale * loss_l2 + \
                        args.r_bn * loss_r_bn_feature.to(loss_l2.device)

            loss_r_bn_feature.to(t_device)
