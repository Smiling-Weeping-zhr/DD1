...
def main():
    ...
    net = nn.DataParallel(net).to(args.device) if args.device == 'cuda:0' else net.to(args.device)
    net.train()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(net, ssh, args, epoch)

        if epoch % 10 == 0 or epoch == args.epochs - 1:
            top1 = validate(net, args, epoch)
        else:
            top1 = 0
        ...
        save_checkpoint({
            'epoch': epoch + 1,
            'state_dict': {'net': net.state_dict(), 'head': head.state_dict()},
            'best_acc1': args.best_acc1,
            'optimizer' : optimizer.state_dict(),
            'scheduler' : scheduler.state_dict(),
        }, is_best, args=args)

def adjust_bn_momentum(net, iters):
    for m in net.modules():
        if isinstance(m, nn.BatchNorm2d):
            m.momentum = 1 / iters


def train(net, ssh, args, epoch=None):
    objs = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for batch_idx, batch_data in enumerate(args.train_loader):
        ...
        for accum_id in range(accum_step):
            ...
            if args.shared is not None:
                inputs_ssh, labels_ssh = rotate_batch(partial_images, args.rotation_type)
                inputs_ssh, labels_ssh = inputs_ssh, labels_ssh.to(inputs_ssh)
                output_ssh = ssh(inputs_ssh)
                # output_ssh = F.log_softmax(output_ssh/args.temperature, dim=1)
                output_ssh = F.log_softmax(output_ssh, dim=1)
                labels_ssh = F.one_hot(labels_ssh.long(), num_classes=4)
                # labels_ssh = F.log_softmax(labels_ssh, dim=1)
                # Maybe KL divergence should not be used to calculate the loss for a four-class classification
                # Keep the loss function consistent.
                # ipdb.set_trace()
                loss += loss_function_kl(output_ssh, labels_ssh.float()) 
    ...
    printInfo = 'TRAIN Iter {}: lr = {:.6f},\nloss = {:.6f},\t'.format(epoch, scheduler.get_last_lr()[0], objs.avg) + \
                'Top-1 err = {:.6f},\n'.format(100 - top1.avg) + \
                'Top-5 err = {:.6f},\n'.format(100 - top5.avg) + \
                'train_time = {:.6f}\n'.format((time.time() - t1))
    print(printInfo)
...
def save_checkpoint(state, is_best, args=None,epoch=None):
    if epoch is None:
        path = args.output_dir + '/' + f'checkpoint_{args.shared}.pth.tar'
    else:
        path = args.output_dir + f'/checkpoint_{args.shared}_{epoch}.pth.tar'
    torch.save(state, path)

    if is_best:
        path_best = args.output_dir + '/' + f'model_best_{args.shared}.pth.tar'
